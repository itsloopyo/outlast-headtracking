...
// Puts the lean back inside the configured limits after the zoom scaling above has
// multiplied it.
//
// The processor clamps the pose on its way out, and ScaledForZoom then multiplies it, so
// on any frame the game draws WIDER than its unzoomed angle the factor is above 1 and the
// eye leaves the box the player configured. Those limits are the only thing keeping the
// eye inside the player's body, so the clamp has to be the last thing that touches the
// lean rather than the second-to-last - the engine-boundary conversion comes first and
// the game-specific limits come after it.
